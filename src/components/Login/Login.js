...
const Login = (props) => {
  ...
  const [clgInputIsValid, setClgInputIsValid] = useState();
  ...
  useEffect(() => {
    const identifier = setTimeout(() => {
      ...
      setFormIsValid(
        emailIsValid && passwordIsValid && enteredClgName.trim().length > 0
      );
    }, 500);
  ...
  const validateCLgNameHandler = () => {
    setClgInputIsValid(enteredClgName.trim().length > 0);
  };
  ...
  const submitHandler = (event) => {
    ...
    } else if (!clgInputIsValid) {
      clgInputRef.current.activate();
    } else {
